import streamlit as st import requests import pandas as pd from datetime import datetime from collections import defaultdict from sklearn.ensemble import RandomForestClassifier from sklearn.model_selection import train_test_split from sklearn.metrics import classification_report import time
...
def load_matches(): urls = ["https://raw.githubusercontent.com/openfootball/football.json/master/2024-25/fr.1.json"] all_matches = [] for url in urls: r = requests.get(url) if r.status_code == 200: data = r.json() league = url.split('/')[-1].split('.')[0] for match in data['matches']: match['league'] = league all_matches.extend(data['matches']) return all_matches
...
def extract_features(matches, rankings): data = [] for match in matches: if 'score' not in match or not match['score'].get('ft'): continue
...
def predict_upcoming_matches(model, matches, date_input, rankings): predictions = [] for match in matches: if match.get('date') != date_input or 'score' in match and match['score'].get('ft'): continue
...
st.title("Football Match Predictor") st.write("Enter a date to predict matches with over 2.5 goals")

date_input = st.date_input("Choose a date") run = st.button("Start Prediction")

if run: with st.spinner("Training model and making predictions..."): matches = load_matches() rankings = build_rankings(matches) df = extract_features(matches, rankings)
...
if df.empty:
        ...
        if results:
            ...
            st.warning("No strong predictions (prob >= 0.75) found for the selected date.")
